defmodule GenEnum do
  ...
  defmacro defenum(quoted_module, quoted_database_type, quoted_values) do
    ...
    quote do
      require EctoEnum
      EctoEnum.defenum(unquote(module).EctoEnum,
                       unquote(database_type),
                       unquote(values))

      defmodule unquote(module).Items do
  ...
  defp define_enum_items(items) do
    items
    |> Enum.reduce(quote do end, fn(enum_item, acc) ->
      quote do
        unquote(acc)
  ...
  defp define_enum_typespec(enum_atoms) do
    {:@, [context: Elixir, import: Kernel],
     [
       {:type, [context: Elixir],
        [{:::, [], [{:t, [], Elixir}, define_algebraic_type(enum_atoms)]}]}
     ]}
  end
  ...
  defp validate_module(atom) when is_atom(atom) and (atom != nil) do
    #
    # TODO : !!!
    #
    :ok
